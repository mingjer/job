...
echo "调用ansible的部署脚本"
ansible-playbook -i /etc/ansible/inventory/${environment} -f 1 /etc/ansible/playbook/core/pay-common.yml -e "jar_file=${jar_file}" -e "jar_file_name=${jar_file_name}" -e "jar_name=${jar_name}" -e "jar_port=${jar_port}" -e "host=${host}" -e "start_bash=${start_bash}" -e "stop_bash=${stop_bash}"