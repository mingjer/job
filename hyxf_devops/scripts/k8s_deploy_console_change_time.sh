...
echo ---处理k8s的部署脚本---
cp /etc/ansible/k8s/common_springboot_console_change_time.yml $WORKSPACE/${service_name}_${environment}.yml
...
sed -i "s#MAX_MEM#${MAX_MEM}#g" $WORKSPACE/${service_name}_${environment}.yml
sed -i "s#51xf_time#${change_time}#g" $WORKSPACE/${service_name}_${environment}.yml
